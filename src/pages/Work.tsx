        ...
        <div className="block justify-center items-center py-5 text-white text-center">
          ...
          <div className="flex flex-wrap justify-center items-center gap-8 pt-8">
            {Portfolio.map((item, idx) => (
              <div key={idx} className=" relative items-center flex justify-center bg-glass w-96 h-96 group transition rounded-md">
                <img src={item.image} className="group-hover:scale-90 transition duration-300 text-white object-contain w-9/12 h-full" alt="" />
                <div className="absolute bg-white w-96 h-96 text-center bg-opacity-0 group-hover:bg-opacity-10 transition-all duration-300">
                  <a href={item.link} className='w-12 h-12 bg-glass group-hover:flex  group-hover:justify-center group-hover:items-center text-lg hidden'><BsEyeFill /></a>
                </div>
              </div>
            ))}
          </div>
